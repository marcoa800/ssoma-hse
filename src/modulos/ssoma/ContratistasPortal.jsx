...
import { ExportContratistasModal } from './ExportContratistasModal.jsx';
...
const regToRow = (nombreDe) => (r) => ({
  "Fecha": r.fecha || "",
  "Tipo": TIPO_LBL[r.tipo] || r.tipo,
  "Contratista": nombreDe(r),
  "Título / Descripción": r.titulo || r.descripcion || "",
  "Lugar / Categoría": r.lugar || r.categoria || "",
  "Estado": r.tipo === "documento" ? "" : (r.estado || ""),
  "Medida correctiva": r.medida_correctiva || "",
  "Foto / Archivo": (r.foto_urls && r.foto_urls[0]) || r.archivo_url || "",
});
function pdfRegistrosPortal(regs, nombreDe, titulo, filename) {
  const doc = new jsPDF();
  const pw = doc.internal.pageSize.getWidth(), ph = doc.internal.pageSize.getHeight();
  doc.setFillColor(15, 23, 42); doc.rect(0, 0, pw, 30, "F");
  doc.setFillColor(124, 58, 237); doc.rect(0, 28, pw, 2, "F");
  doc.setTextColor(255, 255, 255).setFontSize(14).setFont("helvetica", "bold");
  doc.text("REPORTE DE CONTRATISTAS", pw / 2, 13, { align: "center" });
  doc.setFontSize(9).setFont("helvetica", "normal");
  doc.text(titulo, pw / 2, 22, { align: "center" });
  doc.setTextColor(0, 0, 0);
  autoTable(doc, {
    startY: 36,
    head: [["Fecha", "Tipo", "Contratista", "Descripción", "Estado"]],
    body: regs.length
      ? regs.map(r => [r.fecha || "—", TIPO_LBL[r.tipo] || r.tipo, nombreDe(r), (r.titulo || r.descripcion || "").slice(0, 70), r.tipo === "documento" ? "—" : (r.estado || "—")])
      : [["Sin registros en el período", "", "", "", ""]],
    headStyles: { fillColor: [124, 58, 237], textColor: [255, 255, 255], fontStyle: "bold", fontSize: 9 },
    styles: { fontSize: 8 }, alternateRowStyles: { fillColor: [248, 248, 252] },
  ...
  for (let i = 1; i <= total; i++) { doc.setPage(i); doc.setFontSize(7).setTextColor(150); doc.text(`CTG Latam | Pág. ${i} de ${total}`, pw / 2, ph - 8, { align: "center" }); doc.setTextColor(0, 0, 0); }
  doc.save(filename);
}
...
export default function ContratistasModulo({ empresaId }) {
      ...
      <div>
        ...
        {activeTab === "personal" && (
          <div>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden overflow-x-auto">
              <table className="text-sm min-w-[640px]" style={{minWidth:"1100px"}}>
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
            <h4 className="text-white font-semibold text-sm flex items-center gap-2"><FileText size={14} className="text-purple-400" /> Reportes del portal</h4>
            <div className="flex items-center gap-3 flex-wrap">
              <span className="text-[11px] text-gray-500">Código de acceso: <span className="font-mono text-purple-300">{selected.codigo_acceso || "— (sin asignar)"}</span></span>
              <Btn size="sm" variant="ghost" onClick={() => setExpContratista(true)}><Download size={13} /> Por período</Btn>
            </div>
          </div>
          {expContratista && (
            <ExportContratistasModal
              registros={portalRegs}
              contratistas={contratistas}
              lockContratistaId={selected.id}
              onClose={() => setExpContratista(false)}
            />
          )}
          {(() => {
            const selRegs = portalRegs.filter(r => r.contratista_id === selected.id);
            if (!selRegs.length) return <div className="py-8 text-center text-gray-600 text-sm bg-gray-900 border border-gray-800 rounded-xl">El contratista aún no ha subido reportes desde el portal.</div>;
            const tipoLbl = { hallazgo: "Hallazgo", inspeccion: "Inspección", documento: "Documento" };
            const tipoCol = { hallazgo: "red", inspeccion: "blue", documento: "purple" };
            const estCol = { Abierto: "red", "En proceso": "amber", Cerrado: "green" };
            return <div className="space-y-2">{selRegs.map(r => (
              <div key={r.id} className="bg-gray-900 border border-gray-800 rounded-xl p-3">
                <div className="flex items-start gap-2 flex-wrap">
                  <Badge color={tipoCol[r.tipo] || "gray"}>{tipoLbl[r.tipo] || r.tipo}</Badge>
                  {r.tipo !== "documento" && <Badge color={estCol[r.estado] || "gray"}>{r.estado}</Badge>}
                  <span className="text-[11px] text-gray-600 font-mono self-center">{r.fecha || ""}</span>
                  <span className="text-sm text-gray-200 flex-1 min-w-0">{r.titulo || r.descripcion || "—"}</span>
                </div>
                {r.descripcion && r.titulo && <p className="text-xs text-gray-500 mt-1">{r.descripcion}</p>}
                {r.medida_correctiva && <p className="text-xs text-gray-500 mt-1">Medida: {r.medida_correctiva}</p>}
  ...
  return (
    <div>
      ...
      <div className="mb-5 flex items-center gap-3 px-4 py-3 rounded-xl border border-purple-900/40 bg-purple-900/10 flex-wrap">
        <Building2 size={16} className="text-purple-400 shrink-0" />
        <div className="flex-1 min-w-[200px]">
          <div className="text-sm text-purple-300 font-medium">Portal de contratistas</div>
          <div className="text-[11px] text-gray-500">Comparte este enlace; cada contratista entra con su RUC + código de acceso y sube hallazgos, inspecciones y documentos.</div>
        </div>
        {portalRegs.length > 0 && <span className="text-xs text-gray-400">{portalRegs.length} reporte(s) recibidos</span>}
        {portalRegs.length > 0 && <Btn size="sm" variant="ghost" onClick={() => setExpGlobal(true)}><Download size={13} /> Descargar por período</Btn>}
        <Btn size="sm" variant="ghost" onClick={() => { navigator.clipboard?.writeText(`${window.location.origin}/?contratista=${empresaId}`); showToast("Link del portal copiado", "success"); }}>Copiar link</Btn>
      </div>
        ...
        <Modal title={editingC?"Editar contratista":"Nuevo contratista"} onClose={()=>setShowModalC(false)} wide>
          <div className="space-y-3">
            ...
            <div className="pt-2 border-t border-gray-800">
              <p className="text-xs text-gray-600 font-semibold uppercase tracking-wide mb-3">Acceso al portal de contratistas</p>
              <FormField label="Código de acceso (lo usa el contratista junto con su RUC para entrar al portal)">
                <div className="flex gap-2">
                  <Input value={formC.codigo_acceso} onChange={e=>setFormC(p=>({...p,codigo_acceso:e.target.value}))} placeholder="Ej: HGP-4821" />
                  <Btn type="button" size="sm" variant="ghost" onClick={()=>setFormC(p=>({...p,codigo_acceso:"C"+Math.random().toString(36).slice(2,7).toUpperCase()}))}>Generar</Btn>
                </div>
              </FormField>
            </div>
            <div className="pt-2 border-t border-gray-800">
