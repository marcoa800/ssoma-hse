...
export default function FatigaModulo({ workers, empresaId }) {
  ...
  const openEdit = (r) => { setForm({ trabajador_id: r.trabajador_id, fecha_evaluacion: r.fecha_evaluacion, turno: r.turno || "Día", score_epworth: String(r.score_epworth ?? ""), horas_sueno_promedio: r.horas_sueno_promedio ? String(r.horas_sueno_promedio) : "", nivel_actividad: r.nivel_actividad || "Moderado", periodicidad: r.periodicidad || "Anual", observaciones: r.observaciones || "", medico_responsable: r.medico_responsable || "" }); setEditing(r.id); setShowModal(true); };
    ...
    <div>
      <div className="flex items-start justify-between mb-4 gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
          ...
          {subtab === "eval" && <>
            <ExportBtn data={records.map(r => ({ Trabajador: r.trabajadores?.nombre || "", Fecha: r.fecha_evaluacion, Turno: r.turno, "Score Epworth": r.score_epworth, "Horas Sueño": r.horas_sueno_promedio || "", "Nivel Riesgo": r.nivel_riesgo || "", "Próximo control": r.proximo_control || "", Observaciones: r.observaciones || "", Médico: r.medico_responsable || "" }))} filename="fatiga_somnolencia" />
      ...
      {showGuide && <VigGuideModal titulo={VIG_GUIAS.fatiga.titulo} campos={VIG_GUIAS.fatiga.campos} onClose={() => setShowGuide(false)} />}

      {subtab === "controles" && <SeguimientoPanel programa="fatiga" empresaId={empresaId} workers={workers} />}
            ...
            <div key={r.id} className="bg-gray-900 border border-gray-800 rounded-xl p-3.5">
              <div className="flex items-start justify-between gap-2 mb-1.5">
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-gray-500 font-mono mt-0.5">{fmtFecha(r.fecha_evaluacion)} · {r.turno}</div>
                </div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-2">
                <Badge color={nivel.color}>{nivel.label} ({r.score_epworth}/24)</Badge>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-800">
              {["Trabajador", "Fecha", "Turno", "Score Epworth", "Horas Sueño", "Nivel Riesgo", "Próximo Control", "Observaciones", ""].map(h => (
          ...
          <tbody>
            ...
            {!loading && !filtered.length && (
              <tr><td colSpan={9} className="px-4 py-12 text-center text-gray-600 text-sm">{records.length ? "Sin resultados para el filtro aplicado." : "Sin evaluaciones registradas. Usa \"Nueva Evaluación\" para comenzar."}</td></tr>
            )}
          </tbody>
        </table>
        </div>
