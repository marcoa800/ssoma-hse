...
const fs = require('fs');
const BASE = '/Users/marco/Downloads/FORMATO DE INSPECCIONES';
const rd = (f, s) => XLSX.utils.sheet_to_json(XLSX.readFile(`${BASE}/${f}`).Sheets[s], { header: 1, blankrows: true, defval: '' });
const clean = s => String(s ?? '').replace(/\s+/g, ' ').trim();

// ── RE-17 Botiquines: extraer insumos de hoja Administrativo ──
const re17rows = rd('17. SST-RE-17 Formatos Registro de Inspecciขn de Botiquines y maletin.xlsx', 'Administrativo (2)');
const re17items = [];
re17rows.forEach(r => {
  const n = clean(r[0]), desc = clean(r[1]);
  if (/^\d+$/.test(n) && desc && !/INSTRUMENTAL/i.test(desc)) re17items.push({ c: n, n: desc });
});

const serItems = items => items.map(it => `  { c: ${JSON.stringify(it.c)}, n: ${JSON.stringify(it.n)} },`).join('\n');
...
const COMIND_BOTIQUINES = {
  ...
  filasIniciales: 0,
  filasPreset: ${JSON.stringify(re17items.map((it, i) => ({
    item: i+1, descripcion: it.n, fecha_venc: "", stock: "", por_reponer: "", cant_min: "1", estado: "", obs: ""
  })), null, 2)},
};
...
`;
fs.writeFileSync('src/constants/inspecciones-comind-lote1.js', out);
console.log('OK - RE-17 botiquines items extraídos:', re17items.length);
